...
//$pdo = mysql_connect ("u66745.mysql.masterhost.ru","u66745","con9entiotai");
...
class Catalog {
	...
	public function first_query($pdo) {	 	$query = 'SELECT * FROM `h1` WHERE id=21';
		//$pdo->query($query);

		foreach ($pdo->query($query) as $line) {
			//print_r ($line);
			//echo "<div class='div_center'><h1> Кликни по картинке чувак!</h1>";
			//echo "<a href = 'zerro.php?id=$line[0]'>";
			echo "<a href = 'porter1/$line[0]'>";
			//URL Friendly
			echo "<img src='/catalogue/img/porter1.jpg'>";
			echo "</a></div>";
		}
	}//Конец функции

	public function second_query($pdo, $id) {
		//print ("<div class='h1_center'>Кликни по картинкe категории!</div>");
		$query = "SELECT * FROM `h2` WHERE `id_h1` = $id ";
		foreach ($pdo->query($query) as $line) {
			//echo "<div class='div_center2'><a href = 'zerro_next.php?id=$line[0]'>";
			echo "<div class='div_center_all'>";
			echo "<figure class='div_center2'><a href = 'category$line[0]/'>";
			echo("<img src='/catalogue/$line[3]'>");
			echo "<figcaption>";
			echo "$line[1]";
			echo "</figcaption>";
			echo "</a></figure>";
	...
	public function third_query($pdo, $id) {
		//print ("<span class='span_all'>Еще куда нибудь жми!</span>");
		$query = "SELECT * FROM `h3` WHERE `id_h2` = $id ";
		foreach ($pdo->query($query) as $line) {
			//echo "<div class='div_center3'><a href = 'first.php?id=$line[0]'>";
			echo "<div class='div_center3'>";
			echo "<figure class='div_center2'><a href = 'schema$line[0]/'>";
			echo("<img src='/catalogue$line[4]' alt='$line[2] на Портер'/>");
			echo "<figcaption>";
			echo "$line[2]";
			echo "</figcaption>";
			echo "</a></figure>";
